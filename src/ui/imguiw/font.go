// This code has been modified from the code imported by https://github.com/AllenDang/giu/blob/27baeee8d46f9dfaf34ce631edd22e5ebc785179/FontAtlasProsessor.go.
...
import (
	...
	"github.com/Kor-SVS/cocoa/src/resource"
...
const (
	...
	// PretendardGOV_Light   = "PretendardGOV-Light.ttf"
	// PretendardGOV_Medium  = "PretendardGOV-Medium.ttf"
	// PretendardGOV_Bold    = "PretendardGOV-Bold.ttf"

	// PretendardGOV_Black      = "PretendardGOV-Black.ttf"
	// PretendardGOV_ExtraBold  = "PretendardGOV-ExtraBold.ttf"
	// PretendardGOV_SemiBold   = "PretendardGOV-SemiBold.ttf"
	// PretendardGOV_ExtraLight = "PretendardGOV-ExtraLight.ttf"
	// PretendardGOV_Thin       = "PretendardGOV-Thin.ttf"
...
func init() {
	fontFS = resource.GetResourceFS("font")
}
...
func newFontAtlas() *FontAtlas {
	...
	// Pre-register fonts
	fontFile, _ := fontFS.Open(PretendardGOV_Regular)
	fontFileInfo, _ := fontFile.Stat()
	fontFilebytes := make([]byte, fontFileInfo.Size())
	fontFile.Read(fontFilebytes)
	result.registerDefaultFontFromBytes(PretendardGOV_Regular, fontFilebytes, result.fontSize+2)

	fontFile, _ = fontFS.Open(Fontawesome)
	fontFileInfo, _ = fontFile.Stat()
	fontFilebytes = make([]byte, fontFileInfo.Size())
	fontFile.Read(fontFilebytes)
	result.registerDefaultFontFromBytes(Fontawesome, fontFilebytes, result.fontSize+2)
...
func (a *FontAtlas) registerDefaultFontFromBytes(fontName string, fontBytes []byte, size float32) {
	fi := FontInfo{
		fontName: fontName,
		fontByte: fontBytes,
		size:     size,
	...
	a.defaultFonts = append(a.defaultFonts, fi)
}
...
func (a *FontAtlas) rebuildFontAtlas() {
	...
	if len(a.defaultFonts) > 0 {
		...
		for i, fontInfo := range a.defaultFonts {
			...
			if runtime.GOOS == windows {
				xScale, _ := Context.imBackend.ContentScale()
	...
	logger.Trace("Font is rebuilded")
}
